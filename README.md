...
| Commit | What changed |
|---|---|
| _next_ | **Scoring hygiene (Catalyst + emit alignment):** Catalyst Factor 3 now sources from scoring's `etf_direction` (was reading the never-set `etf_tide`) with a **logged** `/api/market/tide` fallback; failed Catalyst factors are marked **NO DATA** (`available:false`) instead of silent NEUTRAL, with a `degraded`/`N of 5 factors` confidence annotation in the UI; Catalyst headers now match the scorer (`UW-CLIENT-API-ID`). `scoreNow` now calls `emitStaleIfChanged`, so the LEVELS-CHANGED badge fires on manual apply/lock/Score-Now too. Five-factor direction math unchanged; level scoring untouched. |
| `ffd6641` | **Removed legacy/dead systems** (deletion only): TradingView **draw** feature (`/draw`, `/draw-qqq`, draw-relay forwarding, UI draw buttons, `DRAW_RELAY_URL`/`ACTION_SECRET`) → replaced by **TASK-PINE**; the **WebSocket** provider stub + REST/WS `/mode` toggle (`USE_WEBSOCKET`); and SCORING.md-flagged dead code (`TOUCH_PCT`, `distanceWeight()`, provider `getDarkPool…`/`getFlowAlerts` methods, `darkPoolShiftTrigger`/`structureBreakWarning`). The "CHART STALE" badge is now a generic "LEVELS CHANGED" indicator. Scoring output byte-identical (`distance100=50` preserved). Added [docs/TASKS.md](docs/TASKS.md). No local-machine integrations remain. |
