const TRIGGER_COLORS = {
  ...
  'time-based':    'text-text-secondary',
  'cascade':       'text-red-400',
  'manual':        'text-teal-400',
  'initial':       'text-text-muted',
  'update':        'text-text-tertiary',
  'overnight-prep':'text-text-tertiary',
  'auto-rescore':  'text-text-tertiary',
}
        ...
        <table className="w-full text-xs font-mono border-collapse">
          ...
          <tbody>
            {rescores.map((event, i) => {
              ...
              return (
                <tr key={i} className="border-b border-border-subtle/60 hover:bg-bg-elevated/40">
                  <td className="py-1 pr-3 text-text-tertiary">{time}</td>
                  <td title={event.trigger || ''} className={`py-1 pr-3 cursor-help ${triggerColor(event.trigger)}`}>
                    {friendlyTrigger(event.trigger)}
                  </td>
                  <td className="py-1 pr-3 tabular-nums">
                    <div className="text-text-primary">{event.price != null ? `$${Number(event.price).toFixed(2)}` : '—'}</div>
                    {nqPrice && <div className="text-text-secondary text-xs">NQ {nqPrice}</div>}
                  </td>
