...
export default function GammaLevelRow({ item }) {
  const meta = ROLE[item.role] || ROLE.strike
  const Icon = meta.icon
  const emphatic = item.role === 'magnet'

  return (
    <div className={`flex items-center gap-2 pl-2 pr-3 py-1 rounded ${emphatic ? 'bg-accent-gamma/10 border border-accent-gamma/30' : ''}`}>
      ...
      <span className={`font-price text-accent-gamma shrink-0 ${emphatic ? 'text-sm font-bold' : 'text-xs font-semibold'}`}>
        {meta.label} {item.strike}
      </span>
      <span className="font-price text-xs text-text-secondary shrink-0">NQ {fmtNq(item.nq)}</span>
