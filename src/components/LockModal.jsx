...
export default function LockModal({ onClose }) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50">
      <div className="bg-bg-elevated border border-border-strong rounded-lg p-6 w-80">
        ...
        {unlocked ? (
          ...
          <>
            <p className="text-text-secondary text-sm mb-4">Enter PIN to unlock draw and control actions.</p>
            <input
              type="password"
              value={input}
              ...
              className="w-full bg-bg-elevated border border-border-strong rounded px-3 py-2 text-text-primary text-sm mb-3 focus:outline-none focus:border-teal-500"
              autoFocus
            />
            {error && <p className="text-red-400 text-xs mb-3">{error}</p>}
            <button
              onClick={handleUnlock}
              className="w-full py-2 bg-teal-700 hover:bg-teal-600 text-text-primary rounded text-sm font-medium"
            >
              Unlock
            </button>
