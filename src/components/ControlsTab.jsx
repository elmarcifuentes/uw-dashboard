...
function SystemStatus({ systemPaused, pausedAt }) {
  ...
  return (
    <div className="bg-bg-card2/60 border border-border-default rounded-lg p-4 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <div className="text-xs text-text-secondary uppercase tracking-wider font-medium">System Status</div>
  ...
  if (!unlocked) {
    return (
      <div className="flex flex-col items-center justify-center py-24 gap-4">
        <div className="text-text-secondary text-sm uppercase tracking-wide">🔒 Controls — PIN Required</div>
        <div className="flex gap-2">
          ...
          <button
            ...
            className="bg-blue-600 hover:bg-blue-500 text-text-primary rounded px-4 py-2 text-sm"
          >
            Unlock
          </button>
        </div>
        {pinError && <div className="text-red-400 text-xs">{pinError}</div>}
        <div className="text-text-muted text-xs mt-2">Same PIN as Levels tab</div>
      </div>
    )
  }
