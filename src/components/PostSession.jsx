...
const TRIGGER_LABELS = {
  'pre-open':       { label: 'Pre-open',  color: 'text-text-secondary' },
  'live':           { label: 'Live',      color: 'text-blue-400' },
  'manual':         { label: 'Manual',    color: 'text-teal-400' },
  'overnight-prep': { label: 'Overnight', color: 'text-text-tertiary' },
  'auto-rescore':   { label: 'Auto',      color: 'text-text-tertiary' },
  'update':         { label: 'Update',    color: 'text-text-tertiary' },
}
function triggerLabel(trigger) {
  ...
  return { label: trigger || '—', color: 'text-text-secondary' }
}

function getEventStyle(eventType) {
  const map = {
    'rescore':           { color: 'bg-bg-elevated',   label: 'Rescore',    text: 'text-text-secondary' },
  ...
  return (
    <div className="bg-bg-card border border-border-subtle rounded-lg p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        {selected !== null && (
          <button onClick={() => setSelected(null)} className="text-xs text-text-muted hover:text-text-secondary">
            ✕ clear
          </button>
        )}
      </div>
          ...
          return (
            <div
              ...
              className={`flex items-start gap-3 rounded px-2 py-1.5 cursor-pointer transition-colors ${
                selected === i ? 'bg-bg-elevated/50' : isKey ? 'hover:bg-bg-elevated/80' : 'hover:bg-bg-elevated/40'
              }`}
            >
              <span className="text-xs text-text-muted font-mono shrink-0 mt-0.5">{timeStr}</span>
              ...
              {event.price != null && (
                <span className="text-xs text-text-muted font-mono shrink-0 ml-auto">
      ...
      {selected !== null && events?.[selected] && (
        <div className="mt-3 border-t border-border-subtle pt-3">
          <div className="bg-bg-card2/50 rounded p-3 space-y-1">
            {Object.entries(events[selected])
              .filter(([k]) => k !== 'id')
              .map(([k, v]) => v != null && (
                <div key={k} className="flex gap-3 text-xs">
                  <span className="text-text-muted w-24 shrink-0">{k.replace(/_/g, ' ')}</span>
  ...
  if (noSessions) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-3 text-text-secondary text-sm">
        ...
        <p>No sessions logged yet. Run <code className="bg-bg-elevated px-1.5 rounded text-text-secondary">npm start</code> to begin logging.</p>
      </div>
    )
  }
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3 flex-wrap pt-3">
        ...
        <select
          ...
          className="bg-bg-elevated border border-border-strong text-gray-200 text-sm rounded px-2 py-1 focus:outline-none"
        >
          {sessions.map(s => (
        ...
        <button
          onClick={exportStory}
          disabled={!story}
          className="ml-auto px-3 py-1.5 text-xs bg-bg-elevated hover:bg-bg-elevated disabled:opacity-40 rounded border border-border-strong text-text-secondary transition-colors"
        >
