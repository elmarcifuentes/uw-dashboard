  ...
  return (
    <div className="space-y-3">
      ...
      <div className={`border rounded-lg px-4 py-3 flex items-center justify-between ${
        ...
        <div>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {noTradeDay && (
              <span className="text-xs text-text-tertiary bg-bg-elevated border border-border-default px-2 py-1 rounded">
                ○ No-trade conditions
              </span>
            )}
          </div>
      ...
      <div className="grid grid-cols-5 gap-2">
        {cards.map(c => (
          <div key={c.label} className="bg-bg-card border border-border-subtle rounded-lg px-3 py-3">
            <div className="text-xs text-text-muted uppercase tracking-wider mb-1">{c.label}</div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-bg-card border border-green-900/30 rounded-lg p-3">
            ...
            <ul className="space-y-1">
              {correct.map((o, i) => (
                <li key={i} className="text-xs text-text-secondary">
                  ...
                  {o.score && <span className="text-text-muted ml-1">({o.score})</span>}
                </li>
              ))}
            </ul>
          ) : (
            <p className="text-xs text-text-disabled">No correct calls this session</p>
          )}
        </div>

        <div className="bg-bg-card border border-red-900/30 rounded-lg p-3">
          ...
          {outcomes.filter(o => o.outcome === 'incorrect').length > 0 ? (
            ...
            <p className="text-xs text-text-disabled">No failed calls this session</p>
          )}
        </div>

        <div className="bg-bg-card border border-indigo-900/30 rounded-lg p-3">
          ...
          {bestLevel ? (
            <div>
              ...
              <div className="text-xs text-text-tertiary">Score {bestLevel.score} · {bestLevel.confidence}</div>
              {bestLevel.dark_pool && (
                <div className="text-xs text-text-muted mt-1">DP {bestLevel.dark_pool?.toFixed(3)}</div>
              )}
            </div>
          ) : (
            <p className="text-xs text-text-disabled">No classified levels this session</p>
          )}
        </div>
      </div>
    </div>
