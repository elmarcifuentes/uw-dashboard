  ...
  return (
    <div className="bg-bg-card border border-border-subtle rounded-lg p-4">
      <div className="flex items-center justify-between mb-3">
        <div>
          <div className="text-xs text-text-tertiary uppercase tracking-wider">Auto vs Manual Levels</div>
          {lastCalculated && (
      ...
      <div className="space-y-0">
        ...
        {levelIds.map(id => {
          ...
          const matchColor = delta === null ? 'text-text-muted' : delta < 0.50 ? 'text-green-500' : delta < 1.50 ? 'text-amber-500' : 'text-red-500'
          ...
          return (
            <div key={id} className="grid grid-cols-5 gap-2 text-xs py-1.5 border-b border-border-subtle/50">
              ...
              <span className="text-right text-text-primary font-mono">{fmt(autoVal)}</span>
              <span className="text-right text-text-tertiary font-mono">{manualVal != null ? fmt(manualVal) : '—'}</span>
              <span className="text-right font-mono text-text-muted">{delta != null ? delta.toFixed(2) : '—'}</span>
