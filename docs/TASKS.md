...
- **TASK-WEBHOOK-AUTH** — done. `/webhook/levels` now requires a shared secret (`WEBHOOK_SECRET` env) via `X-Webhook-Secret` header or `?secret=` query param → 401 + rate-limited reject log on miss; unset var keeps open behavior with a startup warning. The `accept`/`dismiss`/`pending`/`last` siblings are app-internal (frontend-called) and stay open behind the human accept-gate. See the README webhook section.
